...
> ⚠️ 彭章博提到的"AI 自动帮填官网表单"技术风险极高（每家公司表单不同），**MVP 绝对不碰**。

#### ② "AI 改简历是伪需求"

| 李凯风 | 彭章博 | **裁决** |
|--------|--------|---------|
| 不是伪需求，但需结合用户行为 | 是伪需求——一个人只需要几份简历 | **彭章博的前提在校招场景不成立**。校招生投 10+ 大厂，每家侧重点不同。但彭章博的底层逻辑对：交互必须极轻。一键批量生成（SSE 流式）是正确设计。 |

#### ③ AI 面试联动简历

| 李凯风 | 彭章博 | **裁决** |
|--------|--------|---------|
| 先 AI 面试 3 问题 → 再生成简历 | 海投不可能先面再投 | **彭章博正确**。投递和备面解耦。`/optimize` 海投批量生成，`/interview` 约面后准备。李凯风的"轻量面试补全 Profile"作为 v2 功能。 |
...
> 彭章博的三条主线和 PRD 完全吻合，但他可能不知道 AI 简历和面经后端已写完。**现在的瓶颈不是功能缺失，而是入口和导出。**
