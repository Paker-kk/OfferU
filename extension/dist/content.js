...
function makeHashKey(title, company, url) {
...
function extractFromCard(card) {
    ...
    const company = txt(card.querySelector(SEL.COMPANY_NAME)) || txt(card.querySelector(SEL.BOSS_NAME));
    if (!title || !company)
        return null;
...
function extractFromDetail() {
    const title = txt(document.querySelector(SEL.DETAIL_NAME)) ||
        txt(document.querySelector(SEL.JOB_NAME));
    const company = txt(document.querySelector(SEL.DETAIL_COMPANY_NAME)) ||
    ...
    const tags = Array.from(document.querySelectorAll(SEL.DETAIL_TAGS)).map(txt);
    const education = tags.find((t) => /本科|硕士|博士|大专|学历/.test(t)) || "";
    const experience = tags.find((t) => /经验|年|应届|实习/.test(t)) || "";
    const companyTags = Array.from(document.querySelectorAll(SEL.DETAIL_COMPANY_TAGS)).map(txt);
    return {
        title,
        company,
        location,
...
function injectStyles() {
    if (document.getElementById("offeru-ext-styles"))
        return;
...
// ---- 发送采集的岗位到 background ----
function sendToBackground(jobs) {
    chrome.runtime.sendMessage({ type: "JOBS_COLLECTED", jobs });
}
// ---- 列表页：给每个卡片添加采集按钮 ----
function decorateListPage() {
    const cards = document.querySelectorAll(SEL.JOB_CARD);
    cards.forEach((card) => {
        if (card.querySelector(".offeru-collect-btn"))
            return; // 已添加
...
// ---- 列表页：浮动的"一键采集全部"按钮 ----
function addCollectAllFab() {
    if (document.querySelector(".offeru-collect-all-fab"))
        return;
